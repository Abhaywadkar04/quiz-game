...
def use_50_50_lifeline():
    ...
    # Remove two incorrect options from the copy
    incorrect_options = [opt for opt in options_copy if opt != correct_answer]
    removed_option_1 = random.choice(incorrect_options)
    options_copy.remove(removed_option_1)
    removed_option_2 = random.choice(incorrect_options)
    options_copy.remove(removed_option_2)

    # Return a tuple with only two options from the original options list
    remaining_options = [opt for opt in options if opt not in (removed_option_1, removed_option_2)]
    return remaining_options[0], remaining_options[1]
